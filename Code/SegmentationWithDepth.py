...
def get_multiplier(output_tensor):
    class_binary = [[0],[1],[2],[3]] #numero de clases
    #class_binary = [[0],[1]]
...
with dai.Device() as device:
    ...
    while True:
        ...
        if msgs and sync:
            ...
            frame_mix= cv2.applyColorMap(depth_overlay, jet_custom)
            cv2.putText(frame_mix, "Fps: {:.2f}".format(fps.fps()), (2, frame_mix.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, color=(255, 255, 255))
            cv2.imshow("Depth Segmentation", frame_mix)
